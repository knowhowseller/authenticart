...
DROP POLICY IF EXISTS "board_insert_all" ON board_posts;
DROP POLICY IF EXISTS "board_update"     ON board_posts;
DROP POLICY IF EXISTS "board_delete"     ON board_posts;
...
DROP POLICY IF EXISTS "reply_read"   ON board_replies;
DROP POLICY IF EXISTS "reply_insert" ON board_replies;
DROP POLICY IF EXISTS "reply_delete" ON board_replies;
...
CREATE INDEX IF NOT EXISTS idx_board_type    ON board_posts(type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_board_replies ON board_replies(post_id);
...
DROP POLICY IF EXISTS "glr_insert_all"   ON group_lesson_requests;
DROP POLICY IF EXISTS "glr_admin_read"   ON group_lesson_requests;
DROP POLICY IF EXISTS "glr_admin_update" ON group_lesson_requests;
...
-- 클래스 오픈 요청
CREATE TABLE IF NOT EXISTS class_open_requests (
  id               uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS class_open_request_participants (
  id          uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
DROP POLICY IF EXISTS "cor_read"   ON class_open_requests;
DROP POLICY IF EXISTS "cor_insert" ON class_open_requests;
DROP POLICY IF EXISTS "cor_update" ON class_open_requests;
...
DROP POLICY IF EXISTS "corp_read"   ON class_open_request_participants;
DROP POLICY IF EXISTS "corp_insert" ON class_open_request_participants;
DROP POLICY IF EXISTS "corp_update" ON class_open_request_participants;
...
CREATE INDEX IF NOT EXISTS idx_cor_status   ON class_open_requests(status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_corp_request ON class_open_request_participants(request_id);
CREATE INDEX IF NOT EXISTS idx_corp_user    ON class_open_request_participants(user_id);
