...
import { Pin, Eye, EyeOff, Trash2, Plus, X, Pencil } from 'lucide-react'
...
export default function NoticesManager({ notices: initial }: { notices: Notice[] }) {
  ...
  function startEdit(n: Notice) {
    setEditingId(n.id)
    setEditForm({ title: n.title, content: n.content, is_pinned: n.is_pinned, is_published: n.is_published })
  }
  ...
  async function handleEdit(id: string) {
    if (!editForm.title.trim() || !editForm.content.trim()) { toast.error('제목과 내용을 입력해주세요'); return }
    setSaving(true)
    ...
    setNotices(prev => prev.map(n => n.id === id ? { ...n, ...editForm } : n))
    cancelEdit()
  }
    ...
    <div>
      ...
      {notices.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-brand-mist/30 overflow-hidden">
          {notices.map((n, i) => (
            <div key={n.id} className={i > 0 ? 'border-t border-brand-mist/20' : ''}>
              {editingId === n.id ? (
                /* 수정 폼 */
                <div className="p-5 bg-brand-bg/50">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold text-brand-ink">공지사항 수정</p>
                    <button onClick={cancelEdit}><X size={15} className="text-brand-grey" /></button>
                  </div>
                  <div className="space-y-3">
                    <input
                      ...
                      placeholder="제목"
                      className="w-full px-4 py-2.5 rounded-xl border border-brand-mist focus:outline-none focus:ring-2 focus:ring-brand-amber text-sm bg-white"
                    />
                    <textarea
                      ...
                      placeholder="내용"
                      rows={6}
                      className="w-full px-4 py-2.5 rounded-xl border border-brand-mist focus:outline-none focus:ring-2 focus:ring-brand-amber text-sm resize-none bg-white"
                    />
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 text-sm text-brand-grey cursor-pointer">
                        <input type="checkbox" checked={editForm.is_pinned} onChange={e => setEditForm(f => ({ ...f, is_pinned: e.target.checked }))} />
                        상단 고정
                      </label>
                      <label className="flex items-center gap-2 text-sm text-brand-grey cursor-pointer">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={cancelEdit}
                        className="text-sm font-medium px-5 py-2 rounded-full border border-brand-mist text-brand-grey hover:bg-brand-bg"
                      >
                        취소
                      </button>
                    </div>
                ...
                /* 목록 행 */
                <div className={`flex items-center justify-between px-5 py-4 gap-4 ${!n.is_published ? 'opacity-50' : ''}`}>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <button onClick={() => startEdit(n)} title="수정">
