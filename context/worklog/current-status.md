...
- ✅ **IndexNow 99건 재제출**.
- ✅ **엔진 재측정 완료 → 제동 정상화**: `measure.js`+`reinforce.js` 결과 **authenticart 색인 31/83 (37%)**
  (전수 감사 39%와 일치 — 사각지대 해소 확인). `index_rate` 0.69 → **0.37**로 정정되어
  `publish-quota`가 **quota 0 · reinforce 모드**로 전환됐다.
  **→ 내일부터 자동 신규 발행이 멈추고 기존 글 보강 모드로 간다. 이는 설계 의도대로의 정상 동작이다.**
